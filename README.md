...
`EnsureOrdered = true` on the parallel enricher means output order equals input order even when one worker finishes a 10 ms enrichment while another finishes a sub-us passthrough - the fast one waits for the slow one. The classifier sees events in true seq-order (= post-order = real-time order, since posts are serialized through one producer thread).
